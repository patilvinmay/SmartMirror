...
class Ui_MainWindow(object):
    ...
    def update(self):                                                                       #seeker
        if mixer.music.get_busy():
            value=((mixer.music.get_pos()/1000)*100)/self.songlen
    ...
    def setupUi(self, MainWindow):
        ...
        for mp3file in self.mp3gen():                                          #mp3search
            self.songs.append(mp3file)
        ...
        pygame.init()                                                                              #mixer
        mixer.init()
        mixer.music.load(self.songs[0])
        mixer.music.set_volume(self.a)

        self.groupBox = QtWidgets.QGroupBox(self.centralwidget)
        self.groupBox.setGeometry(QtCore.QRect(10, 40, 341, 151))                                 #groupbox
        ...
        self.groupBox_3.setGeometry(QtCore.QRect(720, 10, 351, 211))
        ...
        self.dial.setValue(self.a)                                                            # dial
        ...
        self.groupBox_4.setStyleSheet("background-color:black; border:0;")                                           # group box 4
        ...
        self.scrollArea.setStyleSheet("background-color:black; color: white; border:0;")                  #scroll area
        ...
        self.groupBox_2 = QtWidgets.QGroupBox(self.centralwidget)                              #groupbox_2
        self.groupBox_2.setGeometry(QtCore.QRect(350, 270, 521, 90))
        ...
        self.label_5 = QtWidgets.QLabel(self.groupBox_2)                               #label_5
        f=QtGui.QFont()
        f.setFamily("Calibri")
        f.setPointSize(18)
        f.setBold(True)
        self.label_5.setFont(f)
        ...
        self.lcdNumber.setGeometry(QtCore.QRect(1009, 370, 71, 21))
        self.lcdNumber.display(self.a)                                                                    #lcd no
    ...
    def play(self):
        if self.flag==1:
            ...
            self.flag=0
            if self.flag_2==0:
                songname= tinytag.TinyTag.get(self.songs[self.songno])
                self.songlen=int(songname.duration)
                self.label_5.setText(str(self.songs[self.songno]))
                mixer.music.play()
    ...
    def vol(self):
        self.a=self.dial.value()/100
        mixer.music.set_volume(self.a)


    def nextSong(self):
        if self.songno<(len(self.songs)-1):
            mixer.music.stop()
            ...
            mixer.music.play()
            self.timer.start(1000)
        elif self.songno==(len(self.songs)-1):
            mixer.music.stop()
            self.songno = 0
            mixer.music.load(str(self.songs[self.songno]))
            songname = tinytag.TinyTag.get(self.songs[self.songno])
